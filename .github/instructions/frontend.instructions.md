...
- **Frontend tests are currently skipped.** The Vitest/React Query integration has known issues
  with query cache updates in the test environment. Tests may be written and verified for
  syntactic correctness and imports, but test execution should not be part of the verification
  workflow until the Vitest infrastructure session addresses the cache-update flow. Confirm lint
  and build pass before merging.
