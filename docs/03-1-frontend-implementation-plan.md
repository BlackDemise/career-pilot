...
| Package | Type | Use in CareerPilot |
|---|---|---|
| `react-router-dom` | dependency | Public auth routes, protected application routes, nested shell routes, and redirects |
| `axios` | dependency | Shared HTTP client for `/api/v1/**`, JSON and multipart requests, credentials, and auth interceptors |
| `@tanstack/react-query` | dependency | Server queries/mutations for profile, conversations, CVs, analyses, catalogs, sessions, and reports |
| `react-use-websocket` | dependency | Active interview connection lifecycle and reconnect handling behind a typed feature hook |
| `react-markdown` | dependency | Assistant response rendering when chat markdown arrives in P1 |
| `rehype-sanitize` | dependency | Sanitization of model-controlled markdown before it becomes DOM |
| `lucide-react` | dependency | Accessible icon controls and status indicators |
| `clsx` | dependency | Conditional CSS Module class composition; it is not the styling system itself |
| `vitest` | devDependency | Unit and component test runner integrated with Vite |
| `jsdom` | devDependency | Browser-like DOM environment for component tests |
| `@testing-library/react` | devDependency | Behavior-focused React rendering and queries |
| `@testing-library/user-event` | devDependency | Realistic keyboard, pointer, and form interaction tests |
| `@testing-library/jest-dom` | devDependency | Readable DOM matchers for visibility, roles, values, and document state |
...
| # | Frontend use case | Priority | Backend status | Frontend deliverable |
|---|---|---|---|---|
| F0.1 | App entry point, route tree, error/loading/empty states | P0 | Available | Router, providers, protected layout, not-found and forbidden pages |
| F0.2 | Shared `ApiResponse<T>` client and typed errors | P0 | Available | One fetch client with wrapper parsing and validation-map errors |
| F0.3 | Login, access-token session, logout, refresh-once retry | P0 | Available | Auth context/store, storage policy, session bootstrap, redirect behavior |
| F0.4 | Registration and verification link flow | P0 | Available | Register form, token query parsing, pending-registration page, resend cooldown |
| F0.5 | Forgot password and reset link flow | P0 | Available | Generic request result, pending-reset page, resend cooldown, reset form |
| F0.6 | Authenticated shell and navigation | P0 | Available | Chat, CV, Interview, Profile navigation with mobile layout |
...
| Use cases | Priority | Backend status | Frontend work |
|---|---|---|---|
| 1.1 Create conversation | P0 | Implemented | Conversation list, new conversation action, title state |
| 1.2 Send and receive message | P0 | Implemented | Message timeline, composer, submit lock, request error recovery |
| 1.3 Save and reload history | P0 | Implemented | List/detail loading, selected conversation route, empty states |
| 1.4 Global profile instructions | P0 | Implemented | Profile form for language, style, background, goal, custom instructions |
| 1.5 Career/technology scope | P0 | Implemented by backend prompt | Render refusal as a normal assistant message; do not duplicate policy in UI |
| 1.6 Delete conversation | P0 | Implemented | Confirmed destructive action and list reconciliation |
| 1.7 Streaming, retry, edit, markdown | P1 | Not implemented | Add only after matching backend transport and response semantics exist |
| 1.8 Intent classification | P1 | Not implemented | Add visible context indicators only if API exposes a client-safe contract |
| 1.9 Conversation summaries | P1 | Not implemented | Add summary/loading states once endpoint and persistence exist |
| 1.10 Search, semantic memory, web grounding | P2/P3 | Not implemented | Deferred; no client architecture should assume RAG or retrieval |
...
| Use cases | Priority | Backend status | Frontend work |
|---|---|---|---|
| 2.1 Upload PDF CV | P0 | Implemented | File picker, PDF/type/size feedback, upload progress state |
| 2.2 Extract text | P0 | Implemented | Upload result view; do not expose raw text as the primary result layout |
| 2.3 CV review | P0 | Implemented | Review action, structured assessment, strengths, weaknesses, recommendations |
| 2.4 CV + JD match | P0 | Implemented | JD editor, match score, matched/missing skills, gaps, recommendations |
| 2.5 DOCX/TXT and structured extraction | P1 | Not implemented | Extend accepted-file UI only with backend support |
| 2.6 Section scoring and requirement categories | P1 | Not implemented | Add section comparison views after typed DTOs are defined |
| 2.7 Evidence mapping and ATS analysis | P2 | Not implemented | Deferred evidence view and confidence visualization |
| 2.8 CV rewrite and tailoring | P3 | Not implemented | Deferred; requires a separate editing workflow and explicit backend contract |
...
| Use cases | Priority | Backend status | Frontend work |
|---|---|---|---|
| 3.1 Catalog-backed setup | P0 | Code exists, needs live verification | Load catalogs, dependent role/level/topic selection, duration and min/max budgets |
| 3.2 Compatibility and valid plan | P0 | Code exists, needs live verification | Display backend validation errors; never compute compatibility as authority in React |
| 3.3 Live phased WebSocket interview | P0 | Code exists, needs live verification | Native WebSocket hook, typed event reducer, reconnect/closed states |
| 3.4 Answer deadlines and timeouts | P0 | Code exists, needs live verification | Server-synchronized countdown, timeout submission, disabled stale controls |
| 3.5 State, phase, counters, early ending | P0 | Code exists, needs live verification | Phase indicator and counters driven by server events |
| 3.6 Integrity events and full screen | P0 | Code exists, needs live verification | Full-screen request, visibility/focus listeners, event throttling, neutral status display |
| 3.7 Final evaluation after completion | P0 | Code exists, needs live verification | Completion transition, report fetch, final score and recommendations |
| 3.8 Structured/adaptive/JD setup | P1 | Not available as final contract | Extend setup and event types only after backend DTOs settle |
| 3.9 Fresh web knowledge | P1 | Not implemented | Deferred source/loading/error UI |
| 3.10 Fully adaptive/resume/performance history | P2/P3 | Not implemented | Deferred |
